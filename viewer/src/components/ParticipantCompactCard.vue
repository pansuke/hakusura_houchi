<template>
  <button
    ...
    :class="{
      ...
      'participant-compact-defeated': !participant.alive,
  ...
  >
    <span class="compact-heading">
      ...
      <span v-if="!participant.alive" class="compact-state defeated">戦闘不能</span>
    ...
    <span class="compact-meta">
      <span>PUSH {{ participant.push ?? '-' }}</span>
      <span>POS {{ participant.position ?? '-' }}</span>
    ...
    <span v-if="!participant.alive" class="respawn-note">
...
const hpPercent = computed(() => percent(props.participant.hp, props.participant.max_hp))
